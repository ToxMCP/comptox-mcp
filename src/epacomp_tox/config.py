...
def get_api_key() -> str:
    ...
    api_key = (
        ...
        or settings.ctx_api_key_legacy
        or settings.ctx_api_key_client
    )
    if not api_key:
        raise ValueError(
...
def get_base_url() -> str:
    from epacomp_tox.settings import LEGACY_CTX_BASE_URL, settings
...
def configure_ctx_env(api_key: str | None = None, base_url: str | None = None) -> None:
    ...
    resolved_base_url = base_url or (
        LEGACY_CTX_BASE_URL if settings.ctx_use_legacy else settings.ctx_api_base_url
    )
