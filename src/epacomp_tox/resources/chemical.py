...
class ChemicalResource(BaseResource):
    ...
    def __init__(self, api_key: str):
        ...
        try:
            ...
            logger.info(
                f"Successfully initialized ctx.Chemical with timeout={UPSTREAM_TIMEOUT}s"
            )
                ...
                },
                {
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            "molfile": {
                                "type": "string",
                                "description": "Molfile contents (V2000/V3000)",
                            },
                            "output_format": {
                                "type": "string",
                                "enum": [
    ...
    def search_msready(
        ...
        if normalized == "mass-range":
            ...
            if mass_end is not None:
                kwargs["end"] = mass_end
            result = self._with_retry(lambda: self.client.msready(by="mass", **kwargs))
        else:
            if query is not None:
                kwargs["word"] = query
            result = self._with_retry(
                lambda: self.client.msready(by=search_type, **kwargs)
            )
        return self._ensure_list(result)
    ...
    def get_chemical_property_summary(
        self, dtxsid: str, property_name: Optional[str] = None
    ) -> Any:
    ...
    def batch_get_chemical_predicted_properties(
    ...
    def get_chemical_experimental_properties_by_range(
        ...
        self._raise_properties_unavailable(
            "get_chemical_experimental_properties_by_range"
        )
    ...
    def get_chemical_fate_summary(
        ...
        result = self._with_retry(lambda: self.client.fate_summary(**kwargs))
    ...
    def check_chemical_ghs_links(
        self, source: str, dtxsids: List[str]
    ) -> Dict[str, Any]:
        identifiers = [sid for sid in dtxsids if sid]
        if not identifiers:
            return {"source": source, "results": []}
        result = self._with_retry(
            lambda: self.client.ghs_check_batch(source, identifiers)
        )
        return {
            "source": source,
    ...
    def opsin_convert_name(self, name: str, output_format: str) -> Dict[str, Any]:
        result = self._with_retry(
            lambda: self.client.opsin_convert(name, output=output_format)
        )
        return {
            "name": name,
    ...
    def indigo_convert_molfile(
        self, molfile: str, output_format: str
    ) -> Dict[str, Any]:
        result = self._with_retry(
    ...
    def get_chemical_structure_file(
        ...
        payload = self._with_retry(lambda: self.client.structure_file(**kwargs))
