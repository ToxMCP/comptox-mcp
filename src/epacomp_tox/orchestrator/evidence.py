...
class EvidenceSynthesizer:
    ...
    def synthesize(self, results: Iterable[PredictiveStepResult]) -> EvidenceSynthesis:
        steps: List[PredictiveStepResult] = [
            step for step in results if step.status == "success"
        ]
        if not steps:
            return EvidenceSynthesis(
                confidence_band="Unavailable",
                scores=EvidenceScore(
                    analogue_coverage=0.0,
                    evidence_quality=0.0,
                    predictive_agreement=0.0,
                ),
                narrative="No successful predictive results available for synthesis.",
                recommended_actions=[
                    "Review applicability domain denials",
                    "Re-run orchestration after addressing guardrail failures",
                ],
            )

        analogue_scores = [
            self._extract_score(step, "analogueCoverage") for step in steps
        ]
        quality_scores = [
            self._extract_score(step, "evidenceQuality") for step in steps
        ]
        agreement_scores = [
            self._extract_score(step, "predictiveAgreement") for step in steps
        ]
        ...
        band = self._resolve_confidence_band(
            coverage, evidence_quality, predictive_agreement
        )
        narrative = self._build_narrative(
            band, coverage, evidence_quality, predictive_agreement
        )
    ...
    def _resolve_confidence_band(
        self, coverage: float, quality: float, agreement: float
    ) -> str:
    ...
    def _build_narrative(
        self, band: str, coverage: float, quality: float, agreement: float
    ) -> str:
        return (
    ...
    def _recommended_actions(self, band: str) -> List[str]:
        if band == "Robust":
            return [
                "Proceed with automated dossier generation",
                "Document rationale for regulatory submission",
            ]
        if band == "Limited":
            return ["Seek SME review", "Augment analogue set or supporting evidence"]
        return [
            "Address guardrail failures",
            "Acquire additional data or adjust predictor inputs",
        ]
