...
1. **Author the schema** – drop a Draft 2020-12 schema in the appropriate namespace. Current public coverage includes `common/`, `chemical/`, `cheminformatics/`, `hazard/`, `exposure/`, `bioactivity/`, `workflow/`, `metadata/`, and `predictive/`. Favor the shared schemas in `docs/contracts/schemas/common/` when possible, but prefer dedicated domain namespaces for stable high-value evidence tools.
