...
- **Logging:** `epacomp_tox.server.MCPServer` records upstream metadata (status, request ID, rate-limit headers). Runtime-only values are surfaced through MCP `_meta` so scientific `structuredContent` continues to match its output schema.
