...
| Scientific validation | Offline orchestrator scenario scorecards, evidence-assessment rollups, interop attachment coverage | `python scripts/scientific_validation_report.py --json`, `pytest tests/workflows/test_scientific_validation_report.py`, and `.github/workflows/scientific-validation.yml` |
| Live concordance panel | CTX-backed ToxVal reference cases with expected observed-concordance outcomes and drift detection | `python scripts/live_concordance_panel.py --json`, `pytest tests/workflows/test_live_concordance_panel.py`, and `.github/workflows/scientific-validation.yml` |
