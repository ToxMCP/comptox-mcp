...
### Finding 1: endpoint matrix documentation drift was corrected in this branch

Severity:
- Medium

Why it matters:
- Earlier revisions of `docs/contracts/endpoint-matrix.md` documented `ctx-api/v1` base roots.
- Direct probe tests showed those base roots returned `404`, while the currently functioning CTX probe paths use the non-`v1` endpoint family.
- The current branch updates the endpoint matrix to document the live non-`v1` route families and the packaged metadata bundle.

Evidence:
- Direct probes against the old `v1` base roots returned `404`.
- The patched smoke checker and the live MCP succeed against non-`v1` CTX endpoint paths.
- `docs/contracts/endpoint-matrix.md` now reflects the non-`v1` endpoint families used by the working MCP implementation.
