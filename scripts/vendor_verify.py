...
This gate is ADVISORY on the free-plan repo (no required-status-checks), but the
*bridge* additionally fails closed at runtime on VENDOR_DIGEST_MISMATCH, so a
tampered engine blocks the gate even if CI is only advisory.
"""
...
# This is the SAME digest pin used by every Track-B pilot so the whole fleet
# validates against one byte-identical engine. The load-bearing engine file
# (policy-validator.mjs) is byte-identical to the canonical
# scripts/policy-validator.mjs at this gitSha; index.mjs / run-policy.mjs are the
# flat-layout entrypoint shims.
SPINE_REPO = "https://github.com/ToxMCP/toxmcp-schema-spine"
SPINE_GIT_SHA = "e0a6a0581efd8dfd5b10c2de14435d87769c5944"
VENDORED_AT = "2026-06-25T00:00:00Z"
NOTE = (
    "Digest-pinned vendored copy of the ToxMCP schema-spine policy engine for "
    "the comptox-mcp Track-B scientific-invariants gate (gating the "
    "server-authored prioritize_risk_signals BioactivityExposureRatioRecord "
    "conclusion). vendor:verify recomputes sha256 of every listed file and "
    "hard-fails on mismatch. The load-bearing engine (policy-validator.mjs) is "
    "byte-identical to ToxMCP/toxmcp-schema-spine scripts/policy-validator.mjs at "
    "the pinned gitSha; index.mjs / run-policy.mjs are the flat-layout entrypoint "
    "shims. Byte-authentic copy of the same e0a6a05 vendored tree used by the "
    "metabolomics / proteomics / iata Track-B gates. Do not edit vendored files "
    "by hand."
)
...
def _vendored_files() -> list[Path]:
    ...
    return sorted(
        p for p in VENDOR_ROOT.rglob("*") if p.is_file() and p.name != MANIFEST_NAME
    )
...
def write_manifest() -> int:
    ...
    manifest = {
        ...
        "fileDigests": dict(sorted(file_digests.items())),
    }
...
def verify() -> int:
    ...
    if problems:
        print(
            "[vendor:verify] FAIL — vendored engine integrity check failed:",
            file=sys.stderr,
        )
        for p in problems:
